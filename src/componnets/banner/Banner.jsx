...
const Banner = () => {
  useEffect(() => {
    const animateText = () => {
      ...
      const headlineElement = document.querySelector(".headline_text.tricks");
      ...
      texts.forEach((text, index) => {
        tl.to(headlineElement, {
          opacity: 0,   
          y: 0,            // Fade out the current text first
          duration: 1,             // Duration of fading out
          ease: "elastic.out(1, 0.55)", 
        })
        .set(headlineElement, { textContent: text, opacity: 1, y: 0 }) // Update the text content
        .to(headlineElement, {
          opacity: 0,            // Fade out the text
          y: -50,                // Move out of view
          duration: 1,           // Duration of exiting animation
          ease: "elastic.inOut(1, -0.95)",     // Smooth fade-out effect
